...
const DashboardHome = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        if (role === "librarian") {
          const booksRes = await axios.get("/librarian/books", {
            params: { email: user.email },
          });
          const ordersRes = await axios.get("/librarian/orders", {
            params: { email: user.email },
          });
  ...
  const librarianStatsArray =
    role === "librarian"
      ? [
          {
            title: "Books Added",
            value: stats.totalBooksAdded || 0,
            icon: <FaBook />,
          ...
          },
          {
            ...
            value: stats.unpublishedBooks || 0,
            icon: <FaBook />,
            bgColor: "bg-red-500",
            iconBg: "bg-red-100 text-red-600",
          },
          {
            ...
            value: stats.totalLibrarianOrders || 0,
      ...
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="text-center bg-[var(--color-bg)] rounded-2xl p-8 shadow-md space-y-4">
          <h1 className="text-3xl sm:text-4xl font-bold">
            Welcome back,{" "}
            <span className={`ml-2 ${roleColor}`}>
              {user.displayName || "User"}
            </span>
          </h1>
          <p className="description text-base sm:text-lg md:text-lg max-w-3xl mx-auto">
            Manage your profile, books, wishlist, orders, payments, and reviews
            from a single place. Your dashboard gives you full control with a
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {statsArray.map((stat) => (
                ...
                >
                  <div
                    className={`w-14 h-14 flex items-center justify-center rounded-full mr-6 ${stat.iconBg} text-2xl`}
                  >
                    {stat.icon}
                  </div>
                  <div>
